...
bitmasks, repeated/array fields, nested objects, inline struct
arrays, and four API surfaces (`GameStatus`, `PublicEvent`,
`TeamEvent`, `CommunityEvent`) are working and tested against a live
game. The `lootDefinition` sub-struct with all 14 nested array fields
is fully readable, including object arrays with inline C++ struct
elements. Chest type enums (Int32 ↔ string) are handled
bidirectionally for both get and set. CommunityEvent uses
string-search resolution (not AOB) with vtable-marker validation.
...
5. **`core/Struct.lua`** reads/writes struct elements using a metadata
   template. It handles nested objects (pointer-backed containers),
   namespace containers, leaf fields, and Array fields — batch-reading
   all array vector headers in a single `gg.getValues` call for
   performance. Empty arrays and zero-value scalars are suppressed from
   output for clean results.
6. **`api/GameStatus.lua`** ties it together: looks up a field in
...
own field layout. `core/Struct.lua` reads these templates recursively,
supporting nested `Object` containers (pointer-backed sub-structs like
`lootDefinition`), `Array` fields with `elements` (struct arrays) or
`elementType` + `elementStride` (simple typed arrays), and leaf fields.
...
The container type is selected via the `container = "vector"` shadow
set by the API layer. Inline struct elements (`elementStride > 0x8`)
are read directly at `begin + i * stride` without pointer dereference.
Pointer-based elements (`elementStride = 0x8` or default) dereference
each slot as an INT64 pointer to the element struct.

Empty arrays (size=0 or null begin) are suppressed from output —
only fields with actual data appear in results.
...
local part = Nebula.GameStatus.get("vehicleStatus[1].tuningPartPresets[1].equippedParts[1]")

-- Write scalar
Nebula.GameStatus.set("coins", 999)

-- Write nested array element
Nebula.GameStatus.set("vehicleStatus[1].tuningPartPresets[1].equippedParts[1]", "new_part")
...
| `String` | inline or pointer-deref; two encodings (inline/long). `indirect=false` for C++ inline strings |w) |
...
merged with a persistent address cache so cached addresses survive
signature field modifications. The AOB scan always runs regardless
of cache state.

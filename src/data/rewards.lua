...
-- data/rewards.lua
--==================================================
-- Full event-reward reference payload for
-- Nebula.PublicEvent.set("eventRewards", ...): every reward tier of
-- the current public event with its complete reward names
-- (lootDefinition.id — event_reward_All, event_reward_skins,
-- event_reward_paints, event_reward_parts, event_reward_shards* —
-- plus every unlockVehicles / unlockDriverAssets / unlockVehiclePaints
-- / ... list entry).
--
-- Source: extra/rewards.lua (game-serialized EventDefinition JSON,
-- returned verbatim as a string). This file is the SAME payload
-- converted to a Lua table so it can be passed straight to
-- Nebula.PublicEvent.set("eventRewards", Nebula.PublicEvent.rewards)
-- with no JSON parser.
--
-- One deliberate ABI adjustment: the JSON's nested
-- rewardCondition = { criteria = N, type = "..." } is flattened to
-- the bare float N, because the memory ABI models rewardCondition as
-- the float `criteria` member at +0x4 (see
-- metadata/<version>/structs/ConditionalRewardDefinition.lua — the
-- RewardCondition.type enum is not representable at that offset).
-- lootDefinition keys not present in the LootDefinition template are
-- safe no-ops on write (Struct.set skips unknown keys).
--
-- Exposed as Nebula.PublicEvent.rewards (see main.lua). The same
-- table is valid for Nebula.TeamEvent / Nebula.CommunityEvent — they
-- share the identical EventDefinition reward arrays.
--
-- Generated from extra/rewards.lua. Do not edit by hand.
